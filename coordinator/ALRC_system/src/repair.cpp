...
bool Repair::try_local_repair_for_group(int group_id, int k, int l, int failed_rack_id, int block_size,
...
bool Repair::check_and_add_row(std::vector<std::vector<int>>& mat, const std::vector<int>& new_row) {
    int n = mat.size();
    if (n == 0) {
...
bool Repair::construct_recovery_system(
...
bool Repair::gf256_solve(const std::vector<std::vector<int>>& A,
                 const std::vector<std::vector<uint8_t>>& B,
                 std::vector<std::vector<uint8_t>>& X) {
    return gf256_gaussian_elimination(A, B, X);
}

bool Repair::repair_with_lrc_global_solver(const std::unordered_set<int>& failed_block_ids,
                                   const Placement& placement,
                                   MemcachedClient& client,
                                   int k, int l, int g, int failed_rack_id, int block_size,
                                   const std::vector<std::vector<int>>& encode_matrix,
...
bool Repair::repair_and_set_group_first(int failed_block_id,
                                const Placement& placement,
                                MemcachedClient& client,
