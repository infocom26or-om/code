...
bool Single::try_local_repair_for_group(int group_id, int k, int l, int failed_rackId, int block_size,
                                int failed_block_id,
    ...
    */
    ...
    ////////////////////cross_rack/////////////////////////
    if(parity_entry.rack_id != failed_rackId){
    ...
    */    
    ...
    for (int i = data_start; i < data_end; ++i) {
        ...
        ////////////////////cross_rack/////////////////////////
        if(entry.rack_id != failed_rackId){
...
*/                           
    ...
    // 除以丢失块的系数（乘以逆元）
    int lost_coef = encode_matrix[group_id][failed_block_id];
    ...
    // 写回丢失块
    const auto& lost_entry = placement.table.at(failed_block_id);
    std::string recovered(rhs.begin(), rhs.end());
    

/*    
    // 打印恢复后的 block 内容
    std::cout << "[Local Repair] Recovered block_" << failed_block_id << " : ";
...
*/    
    if (!client.set(lost_entry.server_ip, lost_entry.port, "block_" + std::to_string(failed_block_id), recovered)) {
        std::cerr << "[Local] Failed to set recovered block_" << failed_block_id << std::endl;
        return false;
    }
...
//    std::cout << "[Local Repair] block_" << failed_block_id << " repaired successfully using local parity." << std::endl;
    return true;
}

/*
bool Single::check_and_add_row(std::vector<std::vector<int>>& mat, const std::vector<int>& new_row) {
    int n = mat.size();
    if (n == 0) {
...
bool Single::construct_recovery_system(
...
bool Single::gf256_solve(const std::vector<std::vector<int>>& A,
                 const std::vector<std::vector<uint8_t>>& B,
                 std::vector<std::vector<uint8_t>>& X) {
    return gf256_gaussian_elimination(A, B, X);
}

bool Single::repair_with_lrc_global_solver(const std::unordered_set<int>& failed_block_ids,
                                   const Placement& placement,
                                   MemcachedClient& client,
                                   int k, int l, int g, int block_size,
                                   const std::vector<std::vector<int>>& encode_matrix,
...
/*
    ...
    std::vector<std::vector<int>> matrix;
    std::vector<std::vector<uint8_t>> rhs;
    if (!construct_recovery_system(k, l, g, block_size, placement, client, available_block_ids, failed_block_ids, encode_matrix, matrix, rhs)) {
...
bool Single::repair_and_set_group_first(int failed_block_id,
                                const Placement& placement,
                                MemcachedClient& client,
                                int k, int l, int g, int block_size,
                                std::unordered_map<int, std::string>& recovered_data,
    ...
    int failed_rack_id = failed_entry.rack_id;
/*    
...
*/   
    if(try_local_repair_for_group(group_id, k, l, failed_rack_id, block_size, failed_block_id, placement, client, encode_matrix, recovered_data, repair_time_ms))
    	return true;    

 //   return repair_with_lrc_global_solver(remaining, placement, client, k, l, g, block_size, encode_matrix, recovered_data, repair_time_ms);
    return false;
}
